...
def alumi_per_course(request):
    ...
    for course in courses:
        alumni_count = AlumniProfile.objects.filter(course_id=course).count()
...
def calculate_employment_percentage(request):
    ...
    employed_count = AlumniProfile.objects.filter(current_job__isnull=False).count()
...
def analyze_job_course_relation(request):
    alumni_count = AlumniProfile.objects.count()
    related_job_count = CurrentJob.objects.filter(alumni__course_id=models.F('alumni__course_id')).count()
...
def analyze_alumni_with_jobs_per_course(request):
    ...
    for course in courses:
        alumni_count = AlumniProfile.objects.filter(course_id=course).count()
        employed_count = CurrentJob.objects.filter(alumni__course_id=course).count()
...
def analyze_alumni_with_jobs_within_six_months(request):
    ...
    for course in courses:
        ...
        for graduate in graduates:
            ...
            has_job_within_six_months = CurrentJob.objects.filter(alumni=graduate.alumni, start_date__lte=six_months_after_graduation).exists()
...
def analyze_employment_gap(request):
    ...
    for alumnus in alumni:
        previous_jobs = PreviousJob.objects.filter(alumni=alumnus).order_by('-end_date')
        currentjob = CurrentJob.objects.filter(alumni=alumnus).first()

        if previous_jobs and currentjob:
            latest_end_date = previous_jobs[0].end_date
            gap = (currentjob.start_date - latest_end_date).days
...
def analyze_promotion_rates(request):
    ...
    for alumnus in alumni:
        current_jobs = CurrentJob.objects.filter(alumni=alumnus).order_by('start_date')
        previous_jobs = PreviousJob.objects.filter(alumni=alumnus).order_by('end_date')

...
const TableCoursesList = () => {
  ...
  const deleteCurriculum = async (course_id) => {
  ...
  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="max-w-full overflow-x-auto">
        {data.map((item) => (
          <section key={item.cmo_no}>
            <h4 className="mb-6 pt-5 text-xl font-semibold text-black dark:text-white">
              {item.cmo_no} ({item.start_year}-{item.end_year})
            </h4>
                  ...
                  <tr key={course.course_id}>
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <div className="flex items-center space-x-3.5">
                        ...
                        <button
                          className="hover:text-primary"
                          onClick={() => deleteCurriculum(course.course_id)}>
                          <svg
                            className="fill-current"
                            width="18"
                            height="18"

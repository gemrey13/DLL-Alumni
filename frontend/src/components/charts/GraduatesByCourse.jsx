...
const GraduatesByCourse = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${baseURL}/api/graduates-by-course/`, {
        headers: {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (course.course === value) {
        setOptions((prevOptions) => ({
          ...
          xaxis: {
            ...prevOptions.xaxis,
            categories: course.year,
          },
        }));
        setGraduates({
          series: [
            {
              data: course.graduates,
            },
          ],
        });
        break;
  ...
  return loading ? (
    <div className="col-span-12 rounded-sm border border-stroke bg-white px-5 pt-7.5 pb-5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 animate-pulse">
      <Loader />
    </div>
  ) : (
      ...
      <div className="mb-4 justify-between gap-4 sm:flex">
        <div>
          <h3 className="text-xl font-semibold text-black dark:text-white">
            Employed alumni by course and year
          </h3>
        </div>
        <div>
...
export default GraduatesByCourse;

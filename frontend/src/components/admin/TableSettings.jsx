...
export const Table = ({
  ...
  return <div className="max-w-full overflow-x-auto table-wrapper">
      <table className="table">
        ...
        <tbody>
          {rows.map((row, idx) => {
          return <tr key={idx} className="content-center">
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <span className="actions flex grid-cols-2 gap-4">
                    <BsFillTrashFill className="delete-btn cursor-pointer" onClick={() => deleteRow(idx)} />
                    
                    <BsFillPencilFill className="edit-btn cursor-pointer" onClick={() => editRow(idx)} />

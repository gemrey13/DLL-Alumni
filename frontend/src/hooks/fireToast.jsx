...
const createToast = (title, msg, type) => {
  toast.custom(t => <div className={`${t.visible ? 'animate-enter' : 'animate-leave'}
      ...
      <div className="flex">
          ...
          <button onClick={() => toast.dismiss(t.id)} type="button" className="mr-2 box-content rounded-none border-none opacity-100 hover:no-underline hover:opacity-50 focus:opacity-50 focus:shadow-none focus:outline-none text-white" data-te-toast-dismiss aria-label="Close">
    ...
    for (const alertSetting of JSON.parse(alertSettings)) {
      ...
      if (alertSetting.id == "ALL") {
        Object.keys(dataJSON).map(id => {
          const condition = alertSetting.criterion == '0' ? value <= -1 * dataJSON[id][para] : alertSetting.criterion == '1' || alertSetting.criterion == '3' ? value >= dataJSON[id][para] : alertSetting.criterion == '2' ? value <= dataJSON[id][para] : value == dataJSON[id][para];
